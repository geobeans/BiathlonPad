...
import static cn.geobeans.biathlon.App.DD_KEY;
import static cn.geobeans.biathlon.App.ddItems;
...
public final class FileUtil {
    ...
    private final static long TIME_GAP = 5000;
    /**
     * "/"
     */
    ...
    public static final String SD_PATH = Environment
            .getExternalStorageDirectory()+ File.separator + "biathlon_landscape" + File.separator;
    ...
    private static boolean createMyFile(File file) {
        if (file == null || !makeDirs(getFolderName(file.getAbsolutePath()))) {
            ToastUtils.showToast("create folder failed");
            return false;
        }
        if (!file.exists()) {
            try {
                if (file.createNewFile()) {
                    ...
                    try {
                        ...
                        fos.write("时间，#1 风速，风向，#2 风速， 风向，#3 风速，风向，#4 风速，风向，#5 风速，风向，#6 风速，风向，#7 风速， 风向，#8 风速，风向，#9 风速，风向，#10 风速，风向，#11 风速，风向，#12 风速， 风向，#13 风速，风向，#14 风速，风向，#15 风速，风向，气温，气压".getBytes());
                        fos.write("\r\n".getBytes());//写入换行
                    } catch (IOException e) {
                        ToastUtils.showToast("write file header failed");
                        e.printStackTrace();
                    } finally {
                        if (fos != null) {
                            try {
                ...
                ToastUtils.showToast("create file failed");
                e.printStackTrace();
                return false;
            }
        }else{
    ...
    private static PreferencesUtil preferencesUtil = PreferencesUtil.getInstance();

    public static void addTxtToFileBuffered(String content) {
        String filePath = SD_PATH + preferencesUtil.getParam(DD_KEY, ddItems[0]) +
                File.separator + dateformat.format(System.currentTimeMillis()) +
                File.separator + dateformat1.format(System.currentTimeMillis()) + ".txt";
        BufferedWriter out = null;
        try {
            boolean bNew = createMyFile(new File(filePath));
            out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(filePath), true)));
            if(!bNew){
                //当有断点的时候，空3行 lijp
                if(TIME_GAP< System.currentTimeMillis()-TIME_LAST){
//                    out.newLine();
//                    out.newLine();
//                    out.newLine();
                    out.write("\r\n\r\n\r\n");
            ...
            e.printStackTrace();
            ToastUtils.showToast("write file failed");
        } finally {
            try {
                if (out != null) {
                    out.close();
    ...
    public static List<String> getFileNameList(String dirPath) {
        ...
        for (File file : files) {
            if (file.isFile())
                conList.add(file.getName().replace(".txt", ""));
        }
        return conList;
    }

    public static List<String> getFolderNameList(String dirPath) {
        if (TextUtils.isEmpty(dirPath))
            return Collections.emptyList();
        File dir = new File(dirPath);
        File[] files = dir.listFiles();
